...
class XacNhanSFScreen extends React.Component {
  ...
  constructor(props){
    ...
    donhang = database.ref('DonHang').child('0983362357');
  ...
  _submit(){
    if(this.state.quan == '' || this.state.dist == '' || this.state.sonha == '' || this.state.date =='' || this.state.buoi =='' || this.state.ca =='' || this.state.nhucau==''){
      Alert.alert(
          'Lỗi',
   ...
   else if(this.state.quan != '' && this.state.dist != '' && this.state.sonha != '' && this.state.date !='' && this.state.buoi !='' && this.state.ca !='' && this.state.nhucau!=''){
      donhang.push({
  ...
  render() {
    ...
    return(
      ...
      <ScrollView>
        ...
        <View style = {{flexDirection: 'row', marginLeft: 10, marginTop:5}}>
          <Icon name="edit" size={22} color="gray"/>
          <Text style={{fontSize: 14,
            fontWeight: 'bold',
            color: 'gray',
            marginVertical: 7,
            marginLeft: 5,}}>NHU CẦU</Text>
        </View>
        <View>
          <TextInput
            style={styles.textInputGC}
            multiline={true}
            placeholder="Khách hàng cần vệ sinh nệm, sofa, thảm, rèm... và mô tả chi tiết vật dụng cần vệ sinh (kích cỡ, màu sắc, độ sạch bẩn,...)"
            onChangeText={(value) => this.setState({nhucau:value})}/>
        </View>
       ...
       <View style = {{flexDirection: 'row', marginLeft: 10, marginTop:5}}>
        ...
        <View style= {styles.sotien}>
          ...
          <Text style={styles.hienthisotien}>JupViec sẽ liên hệ để báo giá</Text>
        </View>
        <TouchableOpacity
...
export default XacNhanSFScreen;
